              ...
              <div className="field">
                ...
                {!itemized ? (
                  ...
                  <div className="matbox">
                    ...
                    {/*
                      2つの別々の理由でこの形にしている。
                      ①読み上げ: ライブ領域は「先に DOM に在り、あとから中身が変わる」形で
                        ないと発火しない。空のときも描画は保ち（CSS で寸法だけ 0 にする）、
                        display:none にはしない——それではアクセシビリティツリーから外れ、
                        条件マウントと同じ「領域が現れる／消える」になる（実測で status
                        ノードが 3→2 に減った）。
                      ②入力中の跳ね: これを解いているのは常設ではなく「置き場所」。
                        案内を入力欄より下（材料費の合計の直前）へ移したので、消えても
                        キャレットの位置が動かない（移動量 137.7px → 0.0px）。
                        説明対象の数字の隣に来るという利点も兼ねる。
                    */}
